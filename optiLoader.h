...
// You may want to tweak these based on whether your chip is using an internal low-speed crystal
#define CLOCKSPEED_FUSES   SPI_CLOCK_DIV128
#define CLOCKSPEED_FLASH   SPI_CLOCK_DIV32

typedef struct image {
  char image_name[11];      // e.g. "optiboot_diecimila.hex"
  char image_chipname[10];  // e.g., "attiny88"
  ...
  byte hexcode[4161];       // intel hex format image
} image_t;

typedef struct alias {
  char image_chipname[10];
